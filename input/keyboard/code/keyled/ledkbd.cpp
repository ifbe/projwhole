...
/*
  ...
  pixels_y6y7 = new Adafruit_NeoPixel(ROWS*COLS, PIN_LED_Y6, NEO_GRB + NEO_KHZ800);
  pixels_y6y7->begin();
  */
}
...
void ws2812b_press(int x, int y)
{
  ...
  if(pixels_y0y1){
    ...
    for(int j=0;j<COLS*ROWS;j++){
      pixels_y0y1->setPixelColor(j, pixels_y0y1->Color(r, g, b));
    }
    pixels_y0y1->show();
  }
}
...
void ws2812b_release(int x, int y)
{
  ...
  case 1:
    if(0 == pixels_y0y1)return;
    pixels_y0y1->setPixelColor(2*COLS-1-x, 0);
    pixels_y0y1->show();
    break;
  case 2:
    if(0 == pixels_y2y3)return;
  ...
  case 3:
    if(0 == pixels_y2y3)return;
    pixels_y2y3->setPixelColor(2*COLS-1-x, 0);
    pixels_y2y3->show();
    break;
  case 4:
    if(0 == pixels_y4y5)return;
  ...
  case 5:
    if(0 == pixels_y4y5)return;
    pixels_y4y5->setPixelColor(2*COLS-1-x, 0);
    pixels_y4y5->show();
    break;
  case 6:
    if(0 == pixels_y6y7)return;
  ...
  case 7:
    if(0 == pixels_y6y7)return;
    pixels_y6y7->setPixelColor(2*COLS-1-x, 0);
    pixels_y6y7->show();
    break;
  }
*/
